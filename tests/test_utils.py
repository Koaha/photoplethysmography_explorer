...
from src.utils.file_utils import (
    count_rows_quick, get_columns_only, parse_uploaded_csv_to_temp,
    read_window, get_auto_file_path
)
...
class TestSignalProcessing(unittest.TestCase):
    ...
    def test_quick_snr(self):
        ...
        # Low SNR signal
        x_low = 0.01 * np.sin(2 * np.pi * 10 * np.linspace(0, 1, 1000)) + 0.1 * np.random.randn(1000)
