...
from src.callbacks.plot_callbacks import (
    _generate_time_domain_plots, _generate_frequency_plots,
    _generate_dynamics_plots, _generate_dual_source_plots,
    _generate_insights, _generate_file_info, _create_blank_figure
)
...
class TestPlotCallbacks(unittest.TestCase):
    ...
    def test_generate_time_domain_plots(self):
        ...
        fig_raw, fig_ac = _generate_time_domain_plots(
            "time_domain", self.t, self.red, self.ir, self.red_ac, self.ir_ac,
            "RED", "IR", "butter", "bandpass", 2, self.n, self.template, self.theme
        )
        ...
        fig_raw, fig_ac = _generate_time_domain_plots(
            "frequency", self.t, self.red, self.ir, self.red_ac, self.ir_ac,
            "RED", "IR", "butter", "bandpass", 2, self.n, self.template, self.theme
        )
    ...
    def test_generate_frequency_plots(self):
        ...
        fig_psd, fig_spec = _generate_frequency_plots(
            "frequency", self.red_ac, self.ir_ac, self.fs, 2.0, 0.5, ["on"], 
            self.template, self.theme
        )
        ...
        # Test with different tab
        fig_psd, fig_spec = _generate_frequency_plots(
            "time_domain", self.red_ac, self.ir_ac, self.fs, 2.0, 0.5, ["on"], 
            self.template, self.theme
        )
    ...
    def test_generate_dynamics_plots(self):
        ...
        fig_hr, fig_hist, fig_poi, fig_xc = _generate_dynamics_plots(
            "dynamics", "hr", self.red_ac, self.ir_ac, self.fs, "ir", 40, 180, 0.5, 
            ["hr"], self.template, self.theme
        )
        ...
        # Test with different tab
        fig_hr, fig_hist, fig_poi, fig_xc = _generate_dynamics_plots(
            "time_domain", "hr", self.red_ac, self.ir_ac, self.fs, "ir", 40, 180, 0.5, 
            ["hr"], self.template, self.theme
        )
    ...
    def test_generate_dual_source_plots(self):
        ...
        fig_rtrend, fig_coh, fig_liss, fig_avgbeat, fig_sdppg = _generate_dual_source_plots(
            "dual_source", "rtrend", self.red, self.ir, self.red_ac, self.ir_ac, 
            self.fs, self.t, self.template, self.theme
        )
        ...
        # Test with different tab
        fig_rtrend, fig_coh, fig_liss, fig_avgbeat, fig_sdppg = _generate_dual_source_plots(
            "time_domain", "rtrend", self.red, self.ir, self.red_ac, self.ir_ac, 
            self.fs, self.t, self.template, self.theme
        )
    ...
    def test_generate_insights(self):
        ...
        chips_error = _generate_insights(self.red, self.ir, self.red_ac, self.ir_ac, "Test error", self.template)
    ...
    def test_generate_file_info(self):
        ...
        info = _generate_file_info(
            "/path/to/test.csv", 10000, 100, 200, 100, self.fs, "butter", "bandpass", 2,
