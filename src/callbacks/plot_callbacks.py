...
from ..utils.signal_processing import (
    safe_float, safe_int, design_base_filter, apply_chain, 
    estimate_rates_psd, quick_snr, auto_decimation, cross_correlation_lag
)
from ..utils.ppg_analysis import (
    estimate_spo2, compute_hr_trend, r_series_spo2, 
    ms_coherence, avg_beat, sdppg
)
...
class PlotManager:
    ...
    def create_time_domain_plots(self, t, red, ir, red_ac, ir_ac, 
                                red_col, ir_col, family, resp, order, n):
        """
        ...
        fig_ac = self._create_filtered_plot(td, red_ad, ir_ad, red_col, ir_col, 
                                          family, resp, order, decim_eff)
    ...
    def _create_filtered_plot(self, t, red_ac, ir_ac, red_col, ir_col, 
                             family, resp, order, decim_eff):
        """
    ...
    def create_frequency_plots(self, red_ac, ir_ac, fs, spec_win_sec, spec_overlap, show_spec):
        ...
        fig_spec = self._create_spectrogram(red_ac, ir_ac, fs, spec_win_sec, spec_overlap, show_spec)
    ...
    def create_dynamics_plots(self, red_ac, ir_ac, fs, hr_source, hr_min, hr_max, peak_prom, show_adv):
        """
    ...
    def _create_poincare_plot(self, ibis, show_adv):
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=1000.0*ibis[:-1], y=1000.0*ibis[1:], 
                                mode="markers", name="IBI pairs"))
    ...
    def _create_r_trend_plot(self, tB, Rbeats, spo2_beats):
        ...
        if len(tB) > 0:
            if len(spo2_beats) > 0:
                fig.add_trace(go.Scatter(x=tB[:len(spo2_beats)], y=spo2_beats, 
                                    mode="lines+markers", name="SpO₂ (beat)"))
            # Add R-ratio trace on secondary y-axis
            fig.add_trace(go.Scatter(x=tB[:len(Rbeats)], y=Rbeats, yaxis="y2", 
                                    mode="lines", name="R"))
...
class DataProcessor:
    ...
    @staticmethod
    def process_data(path, window, red_col, ir_col, fs, decim_user, family, resp, 
                    order, rp, rs, notch_enable, notch_hz, notch_q, flags):
        """
...
class InsightGenerator:
    ...
    @staticmethod
    def generate_file_info(path, total, start, end, n, fs, family, resp, order, 
                          low_hz, high_hz, notch_on, decim_user, hr_source, hr_min, 
                          hr_max, peak_prom, spec_win_sec, spec_overlap):
        """
...
def register_plot_callbacks(app):
    ...
    def update_plots(path, total_rows, window, red_col, ir_col, fs, decim_user,
        ...
        t, red, ir, red_ac, ir_ac, filt_err = data_proc.process_data(
            path, window, red_col, ir_col, fs, decim_user, family, resp,
            order, rp, rs, notch_enable, notch_hz, notch_q, flags
        )
        ...
        if t is None:
            ...
            return (
                blank_420, blank_420, blank_360, blank_380,   # 1–4
                blank_320, blank_280, blank_280, blank_280,   # 5–8
                msg, [], None,                                # 9–11
                blank_320, blank_300, blank_300,              # 12–14
                blank_300, blank_300                          # 15–16
            )
        ...
        if main_tab == "dual_source":
            fig_rtrend, fig_coh, fig_liss, fig_avgbeat, fig_sdppg = plot_mgr.create_dual_source_plots(
                red, ir, red_ac, ir_ac, fs, t
            )
        else:
        ...
        info = insight_gen.generate_file_info(
            path, total, start, end, len(t), fs, family, resp, order,
            low_hz, high_hz, "on" in (notch_enable or []), decim_user, hr_source, hr_min,
            hr_max, peak_prom, spec_win_sec, spec_overlap
        )
