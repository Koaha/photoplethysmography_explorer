...
from ..utils.file_utils import count_rows_quick, get_columns_only, parse_uploaded_csv_to_temp, get_auto_file_path
...
def register_data_callbacks(app):
    ...
    def load_data(n_clicks_path, upload_contents, upload_filename, file_path):
        ...
        for col in cols:
            col_lower = col.lower()
            if not red_guess and any(pattern in col_lower for pattern in ["red", "r_", "r-", "r ", "red_adc", "red_signal"]):
                red_guess = col
            elif not ir_guess and any(pattern in col_lower for pattern in ["ir", "infrared", "i_", "i-", "i ", "ir_adc", "ir_signal"]):
                ir_guess = col
